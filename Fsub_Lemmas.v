...
Lemma wf_cset_narrowing : forall V U C E A F X,
...
Lemma wf_typ_narrowing : forall V U T E Ap Am F X,
  wf_typ (F ++ [(X, bind_sub V)] ++ E) Ap Am T ->
  ok (F ++ [(X, bind_sub U)] ++ E) ->
  wf_typ (F ++ [(X, bind_sub U)] ++ E) Ap Am T
with wf_pretyp_narrowing : forall V U T E Ap Am F X,
...
Lemma wf_cset_narrowing_typ : forall C1 C2 C E A F X,
...
Lemma wf_typ_narrowing_typ : forall C1 C2 T E Ap Am F X,
  wf_typ (F ++ [(X, bind_typ C1)] ++ E) Ap Am T ->
  ok (F ++ [(X, bind_typ C2)] ++ E) ->
  wf_typ (F ++ [(X, bind_typ C2)] ++ E) Ap Am T
with wf_pretyp_narrowing_typ : forall C1 C2 T E Ap Am F X,
  wf_pretyp (F ++ [(X, bind_typ C1)] ++ E) Ap Am T ->
  ok (F ++ [(X, bind_typ C2)] ++ E) ->
  wf_pretyp (F ++ [(X, bind_typ C2)] ++ E) Ap Am T.
...
(*   ok (F ++ [(Z, bind_typ Q)] ++ E) -> *)
(*   wf_cset (map (subst_cb Z C) F ++ E) A (subst_cset Z C C'). *)
(* Proof with eauto*. *)
(*   intros *. *)
...
Proof with eauto using cv_free_never_universal, wf_cset_over_union; eauto*.
  ...
  - pick fresh y.
    ...
    simpl.
    unshelve epose proof (H0 y _) as SpH0...
    ...
    rewrite <- H7 in H12. inversion H12; subst...
    assert (t0 = {}N) by fnsetdec; subst...
    constructor...
    unfold allbound_typ in *.
    intros.
    destruct (x == y)...
    unshelve epose proof (H8 x _) as [T Hbinds]...
    exists T.
    binds_cases Hbinds...
  - simpl...
...
Proof with simpl_env; auto*.
  ...
  - pick fresh y; assert (y `notin` L) by fsetdec...  
    unshelve epose proof (H0 y _) as H3...
    inversion H3 as [Henv [Hexpr Hwf]]...
    repeat split...
    + inversion Henv...
    ...
    + constructor...
      ...
      * eapply wf_typ_from_wf_env_typ; apply Henv.
      * destruct (H0 x) as [_ [_ HwfT]]...
Admitted.
