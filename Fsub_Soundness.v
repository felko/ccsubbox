...
Proof with eauto using cv_regular, cv_weakening_head.
  ...
  * rewrite x in *.
    ...
    + apply cv_weakening_head with (F := empty ++ F)...
      ...
      -- (* can't happen *) admit.
      -- assert (p = (Y, B)) by admit; subst...
          simpl_env in *.
        assert (E0 = E) by admit; subst...
    + destruct p. simpl_env in *.
      assert (a = Y) by admit; subst.
      assert (b = B) by admit; subst.
      apply cv_env_irrel...
      apply IHHcv...
      inversion x...
      inversion Hwf...
...
Proof with eauto*.
  ...
  {
    dependent induction H1; dependent induction H2; simpl...
    + inversion H4...
    + inversion H4...
  }
  {
  ...
  }
  {
    ...
    destruct (a' == a0); subst...
    {
      assert (a0 `notin` dom E) by admit.
      inversion H2; subst...
      inversion H1; subst...
      apply IHE with (T := T)...
      pose proof (cv_regular E T C2 H9)...
      pose proof (cv_regular E T C2 H9)...
    }
    {
      inversion H1; subst...
