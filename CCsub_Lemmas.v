...
Proof with eauto.
  ...
  - inversion H; subst.
    ...
    destruct (AllBound a ltac:(fsetdec)) as [T B].
    exists T.
    apply bound_typ, B.
  - fsetdec.
Qed.
...
Proof with eauto using var_cv_bound.
  ...
  - apply wf_cset_over_union in H.
    ...
    inversion H; subst...
    rename select (allbound _ _) into AllBound.
    destruct (AllBound x ltac:(fsetdec)) as [T B].
    exists T.
    apply bound_typ, B.
Qed.
...
Proof with eauto*.
  ...
  induction H; intro.
  - inversion H; inversion H0.
  - inversion H3; inversion H4; destruct (X == X0); inversion H6; subst...
    inversion H0; subst.
    assert (X0 ∈ dom Γ) by (apply binds_In in H8; eauto* ).
    apply (H2 H5).
...
Proof with eauto*.
  ...
  intros y yIn.
  destruct (H y ltac:(fsetdec)) as [T B]; eapply binds_In...
Qed.
...
Lemma sub_pure : forall Γ S T,
  Γ ⊢ S <: T ->
  pure_type S <-> pure_type T.
Proof with eauto*.
  intros * Sub.
  split.
  - intro Hpure; induction Hpure; inversion Sub; subst...
    + Case "type_arr".
    ...
    + Case "type_box".
      ...
      rename select (_ ⊢ _ <: _) into Sub'.
      applys sub_regular Sub'.
  - induction Sub; intro Hpure; inversion Hpure; subst...
    1-5: pose proof (IHSub Hpure) as PU; inversion PU.
    + Case "sub_arr".
      pick fresh x and apply type_arr.
      * apply type_capt...
        eapply capt_from_wf_cset.
...
Proof with simpl_env; auto*.
  ...
  - Case "typing_abs".
    ...
    + pick fresh x and apply expr_abs.
      ...
      * destruct (IH x)...
    + constructor...
      eapply typing_cv with (e := (λ (C # R) e1)) (T := exp_cv e1 # ∀ (C # R) T1)...
  ...
  - Case "typing_app".
    ...
    destruct (typing_var_implies_binds _ _ _ Typ2) as [T1' xBinds].
    constructor.
  ...
  - Case "typing_tapp".
    ...
    + apply wf_typ_open_type with (R := T1); inversion Hwf; subst...
      rename select (pure_type _) into Hpure.
  ...
  - Case "typing_unbox".
    ...
    inversion Hwf; rename select (_ ⊢ (_ # _) wf) into WfCR; inversion WfCR; subst.
    repeat split...
    apply expr_unbox.
    eapply capt_from_wf_cset...
    eassumption.
...
Proof with eauto using wf_cset_over_union.
  ...
  - destruct (AtomSet.F.union_1 xIn)...
    ...
    destruct (AllBound x xIn) as [T B].
    exists T...
Qed.
